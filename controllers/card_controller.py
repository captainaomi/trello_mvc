...
def update_one_card(id):
    ...
    if card:
        ...
        # card.date_updated = date.today - this is if we were to have two dates,
        # eg. date_created and date_updated, and we could edit when it's updated
...
def delete_comment(card_id, comment_id):
    ...
    if comment:
        ...
        db.session.commit()
        return {'message': f'Comment {comment.message} deleted successfully'}, 201
    else:
        return {'error': f'Card not found with id {comment_id}'}, 404
